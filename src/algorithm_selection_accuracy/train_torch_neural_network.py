...
def _evaluate_combination(params: dict, X: np.ndarray, y: np.ndarray, use_gpu: bool = False) -> dict:
    np.random.seed(42)
    random.seed(42)
...
def find_hyperparameters_nn(
    ...
    worker_fn = partial(_evaluate_combination, X=X, y=y, use_gpu=use_gpu)
...
def train_and_test_nn_torch(train_data: list[dict], test_data: list[dict], is_wl: bool = True, is_wlc: bool = True, use_gpu: bool = False) -> dict:
    ...
    hyperparam = find_hyperparameters_nn(X_train, y_train, 1, use_gpu=use_gpu)
