...
def _evaluate_combination(params: dict, X:np.ndarray, y:np.ndarray, problems:list[str]) -> dict:
    ...
    n_elements = len(problems) // cv
    for c in range(cv):
        train_idx = np.array(list(range(c * n_elements)) + list(range((c+1) * n_elements, len(problems))))
        val_idx = np.array(range(c * n_elements, (c+1) * n_elements))
        X_train = X[train_idx,:]
        y_train = y[train_idx]
        X_val = X[val_idx,:]
        y_val = y[val_idx]
...
def size_evaluate(param:dict, hyperparams:dict, train_data:list[dict]) -> tuple[int|None,float]:
    ...
    X = np.array([e['features'] for e in new_train_data])
