...
def _evaluate_combination(params: dict, X: np.ndarray, y: np.ndarray, rnd_state:int) -> dict:
    np.random.seed(42)
    random.seed(42)
    ...
    model = MLPClassifier(**params, hidden_layer_sizes=layer_sizes, random_state=rnd_state)
    cv=3
...
def find_hyperparameters_nn(
    ...
    worker_fn = partial(_evaluate_combination, X=X, y=y, rnd_state=rnd_state)
...
def train_and_test_nn(train_data:list[dict], test_data:list[dict], rnd_state:int) -> dict:
    ...
    hyperparam = find_hyperparameters_nn(train_data, rnd_state, 1)

    features = X_train.shape[1]
    layer_sizes = (features, features * 2, features, features // 2)

    clf = MLPClassifier(**hyperparam, hidden_layer_sizes=layer_sizes, random_state=rnd_state)
