...
def get_preprocessed_X(X:np.ndarray, n_features:list[int]) -> dict[int, np.ndarray]:
    pre_processed = {}
    for n in n_features:
        pca = PCA(n, random_state=42)
        pre_processed[n] = pca.fit_transform(X)
    return pre_processed

def train_rnd_forest(X_train:np.ndarray, y_train:np.ndarray, times:np.ndarray, weights:np.ndarray, train_time:int) -> dict:
    ...
    n_features = [20, 40, 60, 80, 100, 120, 140]
    k_features = Categorical('k', n_features)
    ...
    scenario = Scenario(
        cs,
        n_workers=-1,
        walltime_limit=train_time*60,
        n_trials=10000000
    )

    initial_design = HyperparameterOptimizationFacade.get_initial_design(scenario, n_configs=5)

    preprocessor = get_preprocessed_X(X_train, n_features)
...
def train_and_test_rnd_forest(train_data:list[dict], test_data:list[dict], apply_feature_selection:bool, reduction:None|int=None, train_time:int=30) -> dict:
    ...
    if apply_feature_selection:
        feature_indeces = feature_selection(train_data, 15)
    ...
    hyperparam = train_rnd_forest(X_train, y_train, times, weights, train_time)
    k = None
    if 'k' in hyperparam:
        k = int(hyperparam['k'])
    ...
    print('hyperparameters:', hyperparam, 'with k:', k)
