...
def size_evaluate_nn(param:dict, hyperparams:dict, X:np.ndarray, y:np.ndarray, times:np.ndarray) -> tuple[int|None,float]:
    np.random.seed(42)
    random.seed(42)
...
def find_size_nn(X:np.ndarray, y:np.ndarray, times:np.ndarray, hyperparams:dict, is_wl:bool) -> int|None:
    ...
    for comb in tqdm(all_combinations):
        results.append(size_evaluate_nn(comb, hyperparams, X, y, times))
...
def train_and_test_nn(train_data:list[dict], test_data:list[dict], is_wl:bool=True, is_wlc:bool=True) -> dict:
    ...
    hyperparam = find_hyperparameters_nn(X_train, y_train, times, 1)
    # size = find_size_nn(X_train, y_train, times, hyperparam, is_wl)
