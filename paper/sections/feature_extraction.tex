...
\begin{figure}[t]
    \centering
    \includegraphics[width=.9\textwidth]{figures/wl.drawio.pdf}
...
To use the WL algorithm for feature extraction, we repeat the colour refinement process for a pre-defined number of steps on a single graph, then we extract features by creating a histogram where, for each colour, we count its occurrences in the graph. Then, the final feature vector is created by concatenating the histograms after each iteration. A depiction of the execution of the WL algorithm for feature extraction (with one aggregation step) can be seen in Figure \ref{fig:wl-feature-exec}. The WL features have been proven effective in many fields such as cheminformatics and computer vision. For a comprehensive review of the WL features for ML, we will refer to Morris et al. \cite{morris2023weisfeiler}.

Note that while the standard \texttt{WL} feature extraction process typically concatenates histograms from all iterations, we deliberately utilize only the final histogram. This approach is necessitated by the scale of the graphs extracted from constraint instances; even after preprocessing, the number of unique colors often reaches the thousands. Concatenating these would yield extremely high-dimensional feature vectors, potentially degrading the performance of downstream ML models due to the curse of dimensionality.

Furthermore, as established in the analysis of Graph Isomorphism Networks (GIN) \cite{Xu2018}, the final iteration of a WL-style message-passing scheme theoretically encodes the comprehensive subtree structure. Although \cite{Xuetal2018} focuses on neural architectures, the underlying principle holds for discrete \texttt{WL} features and the final labels represent the most refined partition of nodes, implicitly encapsulating the structural information from preceding iterations. Consequently, in the remainder of this work, \texttt{WL} features refer exclusively to the histogram extracted at the final aggregation step. A comprehensive evaluation of concatenated feature vectors remains a subject for future research.

\subsection{Enhancing Features' Expressiveness}
The standard WL features are designed for graphs without any edge or node information. However, in our case, we have both node and edge-level information we can use to better represent the input graph. For this reason, we include two extensions to the base algorithm:
\begin{itemize}
...
\end{itemize}
These extensions are standard have have been proposed in a similar manner in previous works such as \cite{Togninalli2019-qa}.
...
\begin{algorithm}[ht]
\scriptsize
\caption{\texttt{WLc} Algorithm for Feature Extraction. The \texttt{WLce} version will be different only in the aggregation step where also the edge information will be included.}
\label{alg:wl-cut}
\begin{algorithmic}[1]
...
\begin{figure}[t]
    \centering
    \includegraphics[width=.9\linewidth]{figures/wlc.drawio}
    \caption{Application of the cut trick on two occurrences of the \texttt{all\_different} constraint (top and bottom). Constraint nodes are in purple, variable nodes in yellow and parameter nodes in blue. First, we ignore colours from nodes with incoming edges (1), then we generate all pairs and update the colour of the \texttt{all\_different} node by aggregating it with the set of all pairs (2) and then we generate the pairs histogram by counting all the occurrences of the pairs (3). In this example, since both constraints share the same type of incoming nodes, they will be coloured in the same way. However, having a different node degree will result in a different pair histogram.}
    \label{fig:cut-trick}
\end{figure}
...
During the extraction process, different graph will generate different colours and only a subset of those will be shared between different graphs. This will result in features sets with different histogram sizes and with different colours in them. To train a ML model we need to standardise the histogram making them share the same colours. To do so, we can collect all colours from all histograms and add a 0 coefficient to all colours missing in a histogram. 
