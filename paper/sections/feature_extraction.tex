...
Note that while the standard WL feature extraction process typically concatenates histograms from all iterations, we deliberately utilize only the final histogram. This approach is necessitated by the scale of the graphs extracted from constraint instances; even after preprocessing, the number of unique colors often reaches the thousands. Concatenating these would yield extremely high-dimensional feature vectors, potentially degrading the performance of downstream ML models due to the curse of dimensionality.

Furthermore, as established in the analysis of Graph Isomorphism Networks (GIN) \cite{Xuetal2018}, the final iteration of a WL-style message-passing scheme theoretically encodes the comprehensive subtree structure. Although \cite{Xuetal2018} focuses on neural architectures, the underlying principle holds for discrete WL features and the final labels represent the most refined partition of nodes, implicitly encapsulating the structural information from preceding iterations. Consequently, in the remainder of this work, WL features refer exclusively to the histogram extracted at the final aggregation step. A comprehensive evaluation of concatenated feature vectors remains a subject for future research.\todo{is it standard then in background. Are we the first one to do it, then here}
...
\end{itemize}
These extensions are standard have have been proposed in a similar manner in previous works such as \cite{Togninalli2019-qa}.\todo{la girerei iniziando la descrizione con "Inspired by X, we have ..." invece di mettere questo dopo la spiegazione delle estensioni}
...
After having done so, and after the aggregation steps have been completed, each cut node's colour can be updated by aggregating it with the set (not multiset) comprised of all its connected, original, node colours. By using a set rather than a multiset, nodes with different degrees but the same types of incoming neighbours will receive identical colours.\todo{non seguo, anche se usi un set teoricamente non riesci a distinguere se hanno 2 o 3 neighbours, no?}
...
\todo{lo fai o è solo una proposta? Nel secondo caso in sezione discussione. Non qui che rovina il flow}

The cut trick can only be applied when node information is present, thus, only to \texttt{WLn} and \texttt{WLne} features. We will call the features where the cut trick is applied \texttt{WLc}\todo{WLc-n?} if only the cut is applied to \texttt{WLn} features and \texttt{WLce} if the cut is applied to \texttt{WLne} features\todo{WLc-ne?}. A formalisation of the \texttt{WLc} features can be seen in Algorithm \ref{alg:wl-cut}, and a graphical depiction of how the cut trick is applied can be seen in Figure \ref{fig:cut-trick}.
