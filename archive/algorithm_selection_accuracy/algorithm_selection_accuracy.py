...
from train_as_accuracy_svc import train_and_test_svc
...
def split_stratified_by_gap(df:pd.DataFrame, rnd_state, current_fold:int, max_fold:int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    idxs = [(train_idx, test_idx) for (train_idx, test_idx) in skf.split(df, df['label'])]
...
def main():
    ...
    features_type:Literal['wlce-1', 'wlce-2', 'wlc-1', 'wlc-2', 'wlcu-1', 'wlcu-2', 'wlceu-1', 'wlceu-2', 'wl-1', 'wl-2', 'wln-1', 'wln-2', 'wle-0', 'wle-1', 'wle-2', 'wlne-1', 'wlne-2', 'fzn2feat', 'combined'] = args.features
    ...
    train_data, test_data = get_features(train_data, test_data, features_type, all_levels, f'data/features/acc-{features_type.replace("-","")}-{fold}-{rnd_state}.csv')
    ...
    if model == 'rnd-forest':
        ...
        res = train_and_test_rnd_forest(train_data, test_data, features_type != 'fzn2feat', is_wlc= 'wlc' in features_type, hyperparams=None)
    elif model == 'svc':
        # with open(output_file) as f:
        #     hyperparams = json.load(f)['hyperparameters']
        #     del hyperparams['size']
        res = train_and_test_svc(train_data, test_data, hyperparams=None)
    elif model == 'nn':
        # with open(output_file) as f:
        #     hyperparams = json.load(f)['hyperparameters']
        res = train_and_test_nn_torch(train_data, test_data, hyperparams=None)
    else:
